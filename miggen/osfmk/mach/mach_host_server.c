...
#pragma pack()
	...
	OutP->Head.msgh_size = (mach_msg_size_t)(sizeof(Reply) - 72) + (((4 * OutP->host_info_outCnt)));
...
#pragma pack()
	...
	RetCode = host_zone_info(convert_port_to_host_priv(In0P->Head.msgh_request_port), (zone_name_array_t *)&(OutP->names.address), &OutP->namesCnt, (zone_info_array_t *)&(OutP->info.address), &OutP->infoCnt);
	if (RetCode != KERN_SUCCESS) {
		MIG_RETURN_ERROR(OutP, RetCode);
	}
#if	__MigKernelSpecificCode
	...
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP)
{
	...
	if ((InHeadP->msgh_id > 221) || (InHeadP->msgh_id < 200) ||
